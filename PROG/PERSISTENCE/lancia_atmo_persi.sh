...
#############################################
# 2a. TODO Wind Direction 0-90 (variable prefix is 'wd')
...
#############################################
# 2b. TODO Wind Direction 45-45 (variable prefix is 'wd')
...
export NbNights=`wc -l $FILE_LIST | cut -d ' ' -f 1`
export ROOT=$ATA_PERS_DIR"/${PREFIX_UC}_TREATED/"
if [ ! -d $ROOT ]; then
  echo "ops $ROOT is missing or is not a directory"; exit 1
fi
export ROOT_WS=$ATA_PERS_DIR"/WS_TREATED/"
